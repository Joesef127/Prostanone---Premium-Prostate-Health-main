...
import { ShoppingBag, Phone, MapPin, CreditCard, Package } from 'lucide-react';
import {
  formatCurrency,
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        {total != null && (
          <p className="text-base font-bold text-primary shrink-0">{formatCurrency(total)}</p>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-1.5 mb-3">
        {status != null && (
          <span className={`inline-flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-full border ${statusStyles[variant]}`}>
            {statusIcons[variant]}
      ...
      <div className="flex flex-col gap-1.5">
        ...
        {items != null && (
          <InfoRow icon={<Package className="w-3.5 h-3.5" />}>
            <span className="line-clamp-2 whitespace-normal">{String(items)}</span>
          </InfoRow>
        )}
