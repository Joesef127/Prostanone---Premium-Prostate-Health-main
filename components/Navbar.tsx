...
const Navbar: React.FC = () => {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled || isOpen ? 'bg-white/90 backdrop-blur-md shadow-md py-4' : 'bg-transparent py-6'
        }`}
    >
          ...
          <div className="hidden md:flex items-center space-x-8">
            {NAV_LINKS.map((link) => (
              <Link
                key={link.path}
                to={link.path}
                className={`text-sm font-medium hover:text-primary transition-colors ${location.pathname === link.path ? 'text-primary font-bold' : 'text-gray-600'
                  }`}
              >
                {link.label}
              </Link>
            ...
            <Link to="/checkout">
              <div className="relative p-2 text-gray-600 hover:text-primary">
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-white z-40 pt-24 px-6 md:hidden flex flex-col items-center space-y-8 h-screen">
          ...
          <Link to="/quiz" className="w-full">
            <Button fullWidth size="lg">Check Prostate Health</Button>
          </Link>
        </div>
      )}
    </nav>
